...
@section('content')
	...
	<section class="main-content dashboard-content">
	    <div class="container">
	         <h1 class="page-heading lighter text-uppercase clearfix">PROJECT DASHBOARD

	      @if(Auth::user()->type=='admin')   <a class="pull-right goto-proj" href="{{url('/coding_dashboard')}}"><i class="fa fa-list"></i>
 Go to Project Coding List</a> @endif</h1>
		       ...
		       $.ajax({
		            ...
		            success: function(returnProjectData){  
		               ...
		               var returnProjectData=JSON.parse(returnProjectData);
		                      ...
		                      var returnProjectData ='<tr><td>'+project.project_title+'</td><td>'+project.proposal_no+'</td><td>'+project.sales_man+'</td><td>'+project.manager+'</td><td>'+project.date+'</td><td><a href="{{url('porject_edit')}}/'+project.proposal_no+'"><button class="btn btn-black btn-stroke bold uppercase">Edit</button></a><a href="{{url('porject_delete')}}/'+project.proposal_no+'"><button class="btn btn-black btn-stroke bold uppercase">Delete</button></a></td></tr>';
