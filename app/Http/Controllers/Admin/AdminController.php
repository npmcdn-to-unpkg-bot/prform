      ...
      public function opportunityProjectPost($proposal_no)
       { 
        ...
        //Everything OK, So input the Data into DB or Update DB
		 ...
		 if(!empty($_FILES['signed_proposal'])){ 		
			if($_FILES['signed_proposal']['name']!=''){				
			
			 
                   $file_name_sigend =  Input::file('signed_proposal');
                   $path_name_sigend  = 'uploads/signed_proposal/'.$project->id;
                   $name_signed = $file_name_sigend->getClientOriginalName();
                   $success = $file_name_sigend->move($path_name_sigend,$name_signed);
                   $signed_proposal = $path_name_sigend.'/'.$name_signed;
                } 
            }				
	    ...
	    if(!empty($_FILES['signed_proposal'])){ 		
		 if($_FILES['signed_proposal']['name']!='')
			   {
				 $project_oppertunity->signed_proposal =  $signed_proposal; 
			   }
                    }			   
